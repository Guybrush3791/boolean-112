...
Oggi concentratevi sul *layout*: create un file di layout in cui inserire la struttura comune di tutte le pagine del sito web (tag head, tag body, ...) eventualmente includendo `header` e `footer` tramite due `components`.

Create poi una rotta per visualizzare la lista di tutti i fumetti e abbellite il tutto sfruttando `SASS`.
